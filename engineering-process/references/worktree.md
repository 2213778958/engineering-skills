...
3. One **other** department window follows one tree `issue:` at a time. Planning department main `issue:` is `none`. Parallel leaves = another 分发 (another 推进 on the planning department). Do not 分发 two implement tickets in one planning turn.
...
9. **Drift audit before activating an existing tree.** Before 分发 to a department on an EXISTING tree, `planning` implement runs the drift audit at dispatch-prep and includes it in the receipt: `git rev-list --count` both directions (branch vs `origin/<default>`) + `git cherry` to count true-unique commits. behind <= 5 → cheap fast-forward refresh (current #23 precedent; preserve the disk-only `PROCESS.md`). behind > 5 AND true-unique commits exist → do NOT bulk-merge: recreate the tree from `origin/<default>` and cherry-pick the carried commits one by one, resolving each as its own reviewable commit; carried arbitration receipts attach to the cherry-picked SHAs (the #24-shaped case). behind > 5 AND no true-unique content → recreate the tree fresh; nothing to preserve. This policy does NOT override a standing arbitration preservation verdict by itself: where a verdict mandates preserving specific carried work, cherry-pick satisfies preservation; outright discard only applies when there is nothing unique to preserve.

> #24 woke with a 36-commit drift behind main (the behind=36 outlier; it froze before the P0 pause chain). The bulk-merge reconciliation produced 6 test-seam failures and consumed the first implement employee's entire iteration budget (no receipt); a cherry-pick rebuild on a fresh tree was the better-shaped option but was not offered. Audit data point 2026-09-23: feat/14 behind=1, feat/15 behind=3, feat/16 behind=1, feat/17 behind=1, feat/18 behind=1, feat/22 behind=1 — all cheap-ff cases.
...
| **implement tree** | `<imported>/worktree/<n>-<slug>`, branch `feat/<n>-<slug>` or `fix/<n>-<slug>` | when planning 分发 an implement ticket; remove after that branch merges to default |
| **acceptance tree** | `<imported>/worktree/pr-<n>`, branch `merge/<n>` | when planning 分发 an acceptance ticket and heads≥2; remove after that acceptance merges to default |
...
## Steps: create (`planning` implement, after pick-ticket, before 分发)
