...
| **acceptance** | staff employees; after receipts push `merge/<this-acceptance>` (heads ≥2) and `gh pr` (only after `check_acceptance.py pre` exit 0) / honor `merge:` (merge commit); close this acceptance only after `check_acceptance.py post` exit 0, or (squash / rebase) the person's confirmation comment is on the ticket (hops.md 3e); single head conflicts → reopen that implement ticket `ready-for-agent`; sessions **notify** | merge `engineering:heads`: create `merge/<n>` if needed, merge heads, worktree add/remove per worktree.md; run `check_acceptance.py pre` after merging heads and `post` after the PR merged | review merge / PR scope (scope list vs `blocked-by`) | run `accept:` (`none` may still open a PR) |
...
- `git push` (code repo): `delivery` **manage** after delivery verify passed; `acceptance` **manage** pushes `merge/<this-acceptance>` only (heads ≥2, before opening the PR; force allowed on that branch only). Never the code repo's default branch. The research synthesis subagent may push the external `research/` repo only (rules.md 10).
