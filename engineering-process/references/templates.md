...
| **acceptance** | staff employees; after receipts push `merge/<this-acceptance>` (heads ≥2) and `gh pr` (only after `check_acceptance.py pre` exit 0) / honor `merge:` (merge commit); close this acceptance only after `check_acceptance.py post` exit 0; sessions **notify** | merge `engineering:heads`: create `merge/<n>` if needed, merge heads, worktree add/remove per worktree.md; run `check_acceptance.py pre` after merging heads and `post` after the PR merged | review merge / PR scope (scope list vs `blocked-by`) | run `accept:` (`none` may still open a PR) |
