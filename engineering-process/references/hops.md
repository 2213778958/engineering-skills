...
This acceptance's PR is already merged (`gh pr view` state `MERGED`; e.g. the planning resume says the user merged it) → skip merge heads, `pre` and `accept:`; go to **Post-merge**. This acceptance is already closed (squash / rebase path below) → run only the close-out steps at the end of this hop.
