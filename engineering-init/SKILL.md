...
| Call | When | Done |
|---|---|---|
| **plan** | New-repo planning / init | Conventions decided and injected; contains and uses graphs on spec; tickets labeled (default 1 source 1 sink); `PROCESS.md` has `mode` and `contract: ready`; `MODELS.md` reported; Canvas folder is the `master/`+`worktree/` container |
...
- Routing a call (including `openhands-sessions` for close-out `list`) → check `../engineering-routing/references/routing-table.md` first: a `registered` row is not routable; stop at the gate (enablement is a patch ticket). Consumer: [scripts/routing_registry.py](scripts/routing_registry.py).
...
3. **Module list.** Same names as the architecture graph nodes later.
...
12. **MODELS.md.** Fill `docs/agents/MODELS.md` per [models-stub.md](references/models-stub.md) n×m grid. Route the call through the registry: [scripts/routing_registry.py](scripts/routing_registry.py) checks `../engineering-routing/references/routing-table.md` (a `registered` row is not routable), then run this harness's sessions **list**. Print the table; ask if the user wants changes.
