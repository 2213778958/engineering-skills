...
- `mode` / `until`: process templates.md **Stop** tables are source of truth. Missing `until:` → `none`. Planning manage writes `until` only when the user names a stop. decide that changes tickets or edges applies only GitHub-native changes (official `blocked-by` edges; an open parent sub-issue blocks its children).
...
| process | planning conversation has not confirmed yet, not report back | if `until:` missing write `until: none`; print `mode` / `until` / `merge` / `verify` / `accept` + MODELS; wait; do not advance yet |
