...
- `mode` / `until`: process templates.md **Stop** tables are source of truth. Missing `until:` → `none`. Planning manage writes `until` only when the user names a stop. 决策 that changes tickets or edges applies only GitHub-native changes (official `blocked-by` edges; an open parent sub-issue blocks its children).
