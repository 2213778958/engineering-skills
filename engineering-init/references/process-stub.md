...
- `mode` / `until`: process templates.md **Stop** tables are source of truth. Missing `until:` → `none`. Planning manage writes `until` only when the user names a stop. A decide hop that changes tickets or edges applies only GitHub-native changes (official `blocked-by` edges; an open parent sub-issue blocks its children).
- `planning` implement creates git worktrees under the Canvas container `worktree/` (canvas.md) **before handoff**. `acceptance` implement removes after merge. Do not POST Canvas `worktree: true`. Do not put a tree path in `working_dir`.
- Planning is a **department**. The window is **manage**. ACP is the Cursor bridge, not a role. An advance request = the user asks to move the project forward, judged by intent (e.g. "推进", "领票", "next ticket") → planning manage (`template: planning`, main `issue: none`). Planning **manage** **hands off** tickets to **other** departments (`delivery` / `acceptance` / `arbitration` / `human`) or runs **decide** (staff `planning implement` + `planning review`; do not run patch). After handoff, stop. Do not watch. Duties: process templates.md **duty table**.
- **Other departments** = delivery / acceptance / arbitration / human (`spawn.py` child). One tree `issue:`. That **manage** staffs **employees** per duty table; wait for receipts; sessions **notify** the planning parent. human does not staff implement/review/verify. Human **manage** talks to the user: how to test and accept, and help; wait for pass/fail. Planning **manage** also talks to the user (advance / mode / MODELS). Do not `spawn.py` for implement / review / verify.
- **Employee** = implement / review / verify / research. Delegate only. Not a conversation window. Manage is the department window, not a Task.
...
- init does not ask `template`. process fills tree `template` / `issue` on handoff.
...
| process | planning conversation has not confirmed yet, not a report back | if `until:` missing write `until: none`; print `mode` / `until` / `merge` / `verify` / `accept` + MODELS; wait; do not advance yet |
