...
6. Check: every claim cites a named source; repo-library-first is visible; every `resources:` path has a manifest row. Web-only answer with a repo hit available → fail, then run the face again and rerun synthesis (new commit SHA).
7. Report: entry md path + faces + synthesis commit SHA + subagent names.
