...
| Unconfirmed decision (a caller read and ran routing for the decision) | stay | this conversation's model; planning / human manage, or `engineering-init` plan / migrate / patch in a user-facing conversation (`parent_conversation_id` empty): run `grilling` until the user confirms; any other caller: stop, report `Missing: decision` upward |
...
- Unconfirmed decision = `stay`; never delegate or dispatch it. Only a conversation that talks to the user asks: planning / human manage, or a user-facing `engineering-init` plan / migrate / patch.
